...
/**
 * Description: Uses a flyweight to hold an entire Hives base stats so a hive
 * does not need to hold true bee objects, only get the base state based on type
...
public class FlyweightDemo implements BeeEnums {
    ...
    static Task[] tasks = { Task.BUILDING, Task.FIGHTING, 
        Task.HARVESTING, Task.IDLE, Task.RESTING };
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Creating 10 bees of random type and with a random task"
                + "using Flyweight,\nif bee type already exists,"
                + " copies flyweight data" + "and only sets new task\n");
        for (int i = 0; i < 10; i++) {
        ...
        System.out.println("Total number of carpenter bees: " 
                + BeeFactory.getBeeCount(Type.CARPENTER));
